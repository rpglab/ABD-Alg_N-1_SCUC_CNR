...
var Vgt{g in GEND, t in LOAD} binary; #>= 0, <=1; #binary; # # startup var (binary var modeled as continuous since it will have binary solution)
...
param reserve_init{GEND,LOAD};
param line_flow_init{BRANCH,LOAD};
...
subject to LineCont51 {j in BRANCH, c in Nk, t in LOAD}:
    ...
    -bus_angle_cont[branch_fbus[j],c,t]) + (1 - S[j,c,t])*M >= 0;

subj to LineCont52 {j in BRANCH, c in Nk, t in LOAD}:
    line_flow_cont[j,c,t]/100 - Nck[c,j]*branch_b[j]*(bus_angle_cont[branch_tbus[j],c,t]
    -bus_angle_cont[branch_fbus[j],c,t])  - (1 - S[j,c,t])*M <= 0;
    
subject to LineCont6 {j in BRANCH, c in Nk, t in LOAD}:
    -line_flow_cont[j,c,t] <= S[j,c,t]*branch_rateC[j]*Nck[c,j];
...
subject to Trans_swtch {c in Nk, t in LOAD}:
	sum{j in BRANCH} (1 - S[j,c,t]) <= 1;
